...
<div class="mx-auto grid max-w-3xl grid-rows-[1fr_auto] gap-2">
	<div class="overflow-hidden rounded-lg border border-gray-200">
		...
		<div class="min-w-full bg-gray-100">
				...
				{#if isJsonContent}
					<table class="min-w-full divide-y divide-gray-200">
						<tbody class="divide-y divide-gray-200">
							{#each (Array.isArray(parsedContent) ? (isExpanded ? parsedContent : parsedContent.slice(0, 5)) : Object.entries(parsedContent)) as item, argIdx}
			...
			{#if (shouldTruncate && !isJsonContent) || (isJsonContent && Array.isArray(parsedContent) && parsedContent.length > 5)}
				<button
