...
export function baseMessageObject(item: unknown): string {
  if (isBaseMessage(item)) {
    ...
    if ("tool_calls" in item) {
      toolCallText = JSON.stringify(item.tool_calls, null, 2);
    }
    if ("type" in item) {
...
export function createDefaultHumanResponse(
  ...
  initialHumanInterruptEditValue: { current: Record<string, string> },
): {
    ...
    if (interrupt.config.allow_accept) {
      Object.entries(interrupt.action_request.args).forEach(([k, v]) => {
        ...
        if (typeof v === "string") {
          ...
          stringValue = JSON.stringify(v, null, 2);
        }
...
export function messageTypeToLabel(message: BaseMessage): string {
  let type = "";
  if ("type" in message) {
